...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderizarComidas() {
        ...
        comidasActivas.forEach(comida => {
            ...
            col.innerHTML = `
                ...
                <span class="d-block ${estaCompletada ? 'text-white' : 'text-subtle'} fw-medium subtexto-fluido">${LS(comida.id === "desayuno" ? "desayuno" : comida.id === "merienda_m" ? "meriendaManana" : comida.id === "almuerzo" ? "almuerzo" : comida.id === "merienda_t" ? "meriendaTarde" : "cena")}</span>
        ...
        metaComidas.textContent = `${totalMeta} ${LS("tiemposAlDia")}`;
        ...
        if (completadas >= totalMeta) {
            btnAddComida.textContent = LS("comidasCompletadas");
            btnAddComida.classList.add('disabled');
        } else {
            btnAddComida.textContent = LS("sumarComida");
    ...
    document.getElementById('btn-guardar-configuracion').addEventListener('click', () => {
        ...
        for (let i = 0; i < activas.length; i++) {
            ...
            if (!actual.inicio || !actual.fin) {
                errorAlert.textContent = `${LS("especificaHoras")} ${LS(actual.id === "desayuno" ? "desayuno" : actual.id === "merienda_m" ? "meriendaManana" : actual.id === "almuerzo" ? "almuerzo" : actual.id === "merienda_t" ? "meriendaTarde" : "cena")}.`;
                errorAlert.classList.remove('d-none');
                return;
            }

            if (actual.inicio >= actual.fin) {
                errorAlert.textContent = `${LS("horaInicioAnterior")} ${LS(actual.id === "desayuno" ? "desayuno" : actual.id === "merienda_m" ? "meriendaManana" : actual.id === "almuerzo" ? "almuerzo" : actual.id === "merienda_t" ? "meriendaTarde" : "cena")}.`;
                errorAlert.classList.remove('d-none');
                return;
            }
            ...
            if (i > 0) {
                ...
                if (actual.inicio < anterior.fin) {
                    errorAlert.textContent = `${LS("incongruencia")} ${LS(actual.id === "desayuno" ? "desayuno" : actual.id === "merienda_m" ? "meriendaManana" : actual.id === "almuerzo" ? "almuerzo" : actual.id === "merienda_t" ? "meriendaTarde" : "cena")}.`;
                    errorAlert.classList.remove('d-none');
                    return;
                }
            }
    ...
    window.addEventListener("lifesyncIdiomaCambiado", renderizarComidas);
});